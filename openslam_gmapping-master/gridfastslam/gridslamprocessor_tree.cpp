...
void  GridSlamProcessor::updateTreeWeights(bool weightsAlreadyNormalized){
  ...
  if (!weightsAlreadyNormalized) {
    normalize();//in gridslamprocessor.ｈxx
  }
  resetTree();
  propagateWeights();
}
...
double GridSlamProcessor::propagateWeights(){
  // don't calls this function directly, use updateTreeWeights(..) !
        ...
        // the accumulated weight of the root　统计所有历史粒子的权重总和
	double lastNodeWeight=0;　
	// sum of the weights in the leafs
	double aw=0;                   

	std::vector<double>::iterator w=m_weights.begin();//已经更新过的权重
	for (ParticleVector::iterator it=m_particles.begin(); it!=m_particles.end(); it++){
		double weight=*w;
		aw+=weight;
		TNode * n=it->node;
		n->accWeight=weight;
		//propagateWeight是一个递归　应该是将一条父亲线上的所有ｗｅｉｇｈｔ相加
