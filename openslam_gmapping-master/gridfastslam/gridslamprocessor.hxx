...
inline void GridSlamProcessor::scanMatch(const double* plainReading){
  ...
  for (ParticleVector::iterator it=m_particles.begin(); it!=m_particles.end(); it++){
    ...
    double score, l, s;
    //得到最优的打分为每一个粒子 在这个过程中对粒子进行了微调　然后将最好的粒子以及得分返回
    //传入一个corrected，在函数里面将对更新过的位置进行赋值
    score=m_matcher.optimize(corrected, it->map, it->pose, plainReading); //in scanmatcher.cpp
    //    it->pose=corrected;
    if (score>m_minimumScore){
      it->pose=corrected;//将更新矫正过的粒子赋值给it 即更新当前粒子
    } else {
	if (m_infoStream){
...
　//计算likelihood score
    m_matcher.likelihoodAndScore(s, l, it->map, it->pose, plainReading);//in scanmatcher.h
    sumScore+=score;
    it->weight+=l;
    it->weightSum+=l;
...
inline void GridSlamProcessor::normalize(){
  ...
  for (std::vector<Particle>::iterator it=m_particles.begin(); it!=m_particles.end(); it++){
    m_weights.push_back(exp(gain*(it->weight-lmax)));//把gian做了exp指数
    wcum+=m_weights.back();//最后统一加到wcum里面
    //cout << "l=" << it->weight<< endl;
  }
  
  m_neff=0;
  for (std::vector<double>::iterator it=m_weights.begin(); it!=m_weights.end(); it++){
    *it=*it/wcum;//所有的已经exp过的权重　统一百分比
    double w=*it;//
    m_neff+=w*w;//所有已经exp过的权重　相乘　最后统一相加
  }
  m_neff=1./m_neff;//
...
inline bool GridSlamProcessor::resample(const double* plainReading, int adaptSize, const RangeReading* ){
  ...
  for (unsigned int i=0; i<m_particles.size(); i++){
    oldGeneration.push_back(m_particles[i].node);//记录的头粒子节点的位置
  }
  
  //m_neff　在normalize 中生成 　重采样界限　该值小于　界限　则重采样
