...
(double srr, double srt, double str, double stt){
  ...
  bool GridSlamProcessor::processScan(const RangeReading & reading, int adaptParticles){
    ...
    OrientedPoint relPose=reading.getPose();//机器人当前位置
    if (!m_count){
    ...
    for (ParticleVector::iterator it=m_particles.begin(); it!=m_particles.end(); it++){
      OrientedPoint& pose(it->pose);//得到粒子pose的引用
      //传入参数粒子位置　　当前位置　上次位置
      pose=m_motionModel.drawFromMotion(it->pose, relPose, m_odoPose);//粒子pose的值被返回的值更新替代　运动模型
    }
    ...
    OrientedPoint move=relPose-m_odoPose;　　//得到位置差值
    move.theta=atan2(sin(move.theta), cos(move.theta));
    m_linearDistance+=sqrt(move*move);
    m_angularDistance+=fabs(move.theta);
    
    // if the robot jumps throw a warning　　如果机器人位置出现的过大的跳动　默认20m
    ...
    m_odoPose=relPose;//更新位置
    
    bool processed=false;

    // process a scan only if the robot has traveled a given distance 只有当机器人移动过一定距离以后或者转动过一定角度 才处理scan
    if (! m_count 
	...
	/*scanMatch 里面实现了计算每个粒子的打分，如果超过预期值则微调一下粒子的位置
	 * 计算每个粒子的权重
	 * 更新每个粒子的各自地图*/
	scanMatch(plainReading);//扫描匹配  in gridslamprocessor.hxx 
	...
	//权重计算 权重没有统一化过　更新权重
	/*归一化权重　计算Neff*/　
	updateTreeWeights(false); // 更新权重  in gridslamprocessor.hxx 
	...
	//重采样
	resample(plainReading, adaptParticles);// in gridslamprocessor.hxx
