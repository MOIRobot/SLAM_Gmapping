...
- @b "~/astep" @b [double] initial search step for scan matching (angular) 初始搜寻step 角度
...
//角度每秒
- @b "~/vth_degree" @b [double] size of one pixel [m]
...
SlamGMapping::SlamGMapping():
  ...
  laser_count_(0), private_nh_("~"), scan_filter_sub_(NULL), scan_filter_(NULL), transform_thread_(NULL),odom_helper_("odom")
{
  seed_ = time(NULL);
  init();
}
...
SlamGMapping::SlamGMapping(ros::NodeHandle& nh, ros::NodeHandle& pnh):
  map_to_odom_(tf::Transform(tf::createQuaternionFromRPY( 0, 0, 0 ), tf::Point(0, 0, 0 ))),
  laser_count_(0),node_(nh), private_nh_(pnh), scan_filter_sub_(NULL), scan_filter_(NULL), transform_thread_(NULL),odom_helper_("odom")
{
  seed_ = time(NULL);
  init();
}
...
SlamGMapping::SlamGMapping(long unsigned int seed, long unsigned int max_duration_buffer):
  ...
  seed_(seed), tf_(ros::Duration(max_duration_buffer)),odom_helper_("odom")
{
  init();
}
...
void SlamGMapping::init()
{
  ...
  //生成一个grid slam processor 对象 源码在opt/ros/include/gmapping 中
  ...
  if(!private_nh_.getParam("tf_delay", tf_delay_))
    ...
    //添加
    if(!private_nh_.getParam("vth_degree", vth_degree_))
    vth_degree_ = 15;
...
SlamGMapping::addScan(const sensor_msgs::LaserScan& scan, GMapping::OrientedPoint& gmap_pose)
{
  ...
  } else 
  {
    ...
    {
      // Must filter out short readings, because the mapper won't
      if(scan.ranges[i] < scan.range_min) //如果激光的数据小于规定的最小数据 则 将数据置为最大数据 把小于扫描最短范围的数据滤出
        ranges_double[i] = (double)scan.range_max;
      else
        ranges_double[i] = (double)scan.ranges[i];//将激光的数据复制到ranges_double
    }
  }
...
{
  //每隔throttle_scans_ 更新一次 用来限制激光流量速度 throttle 节流阀
  laser_count_++;
  ...
  // We can't initialize the mapper until we've got the first scan 初始化一次
  if(!got_first_scan_)
  {
 ...
 里面还调用了 GridSlamProcessor::init ,这个初始化了粒子数，子地图大小 */
  ...
  GMapping::OrientedPoint odom_pose; //oriented 导向的定向的
...
SlamGMapping::updateMap(const sensor_msgs::LaserScan& scan)
{
  ...
  matcher.setlaserMaxRange(maxRange_);//设置激光最大距离
  matcher.setusableRange(maxUrange_);//设置可用的距离
  matcher.setgenerateMap(true);

  GMapping::GridSlamProcessor::Particle best =
          gsp_->getParticles()[gsp_->getBestParticleIndex()];//得到最优粒子
